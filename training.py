...
parser.add_argument("--architecture", type=str, default="resnet18", help="Torchvision models.")
parser.add_argument("--n_inputs", type=int, default=None, help="Number of images. None loads all the available ones.")
parser.add_argument("--num_iters", type=int, default=2, help="Number of training iterations for baseNN.")
...
parser.add_argument("--svi_iters", type=int, default=5, help="Number of training iterations for redBNN.")
parser.add_argument("--hmc_samples", type=int, default=50, help="Number of samples for training using HMC algorithm.")
parser.add_argument("--hmc_warmup", type=int, default=100, help="Number of warmup steps for training using HMC algorithm.")
parser.add_argument("--load", type=eval, default="False", help="Load trained network and evaluate on test data.")
...
dataloaders, input_size, num_classes = load_data(dataset_name=args.dataset, 
                                                 data_dir='examples/imagenette2-320',
...
if args.model=='baseNN':
    ...
    model.train(dataloaders=dataloaders, num_iters=args.num_iters, device=args.device)
    model.save(filename='baseNN', savedir='examples/')
    ...
    model.train(dataloaders=dataloaders, num_iters=args.num_iters, device=args.device, 
                eval_samples=args.n_samples, svi_iters=args.svi_iters, 
                hmc_samples=args.hmc_samples, hmc_warmup=args.hmc_warmup)
    model.save(filename='redBNN', savedir='examples/')
